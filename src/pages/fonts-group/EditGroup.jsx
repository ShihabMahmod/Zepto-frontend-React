...
const UpdateGroup = () => {
  ...
  const fetchGroupDetails = async () => {
    try {
      const response = await axios.get(`http://localhost/api/public/FontGroup.php?action=getGroup&group_id=${id}`);
      const groupData = response.data;

      setGroupName(groupData.group_name);
      const fontIdsArray = groupData.font_ids.split(',');
      const customNamesArray = groupData.custom_names.split(', ');
      const formattedRows = fontIdsArray.map((fontId, index) => ({
        font: fontId.trim(), 
        name: customNamesArray[index]?.trim() || '', 
      }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`http://localhost/api/public/FontGroup.php?_method=PUT&action=updateGroup&id=${id}`, formData, {
        headers: {
  ...
  return (
    <div className="container m-auto my-8 p-6 bg-white shadow-lg rounded-md">
      <h2 className="text-2xl font-bold mb-4">Update Font Group</h2>
